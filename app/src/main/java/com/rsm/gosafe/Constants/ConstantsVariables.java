package com.rsm.gosafe.Constants;

public class ConstantsVariables {

    public static final String SPLASHLAYOUT = "splashLayout";
    public static final String SPLASHLOGO = "splashLogo";
    public static final String APPNAME = "AppName";
    public static final String EMAILEDITTEXT = "emailEditText";
    public static final String PASSWORDEDITTEXT = "passwordEditText";
    public static final String SIGNINBUTTON = "signButton";
    public static final String PASSWORDCHECK = "passwordCheck";
    public static final String ERROR = "This Field Can't be Empty";
    public static final String CHANNELID = "123";
    public static final String MESSAGE = "Hey! I'm in trouble. Need help.";

    public static final int DURATION = 2000;
    public static final int TRANSITIONDURATION = 1000;
    public static final int PERMISSIONCODE = 101;
}
